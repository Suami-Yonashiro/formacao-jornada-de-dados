...
def grafico_100m():
    dados = [(nome, cor, tempos[2]) for nome, (cor, tempos) in RESULTADOS.items()
             if nome != "Bash + AWK"]  # Bash fica de fora (benchmark não confiável em 100M)
    ...
    for bar, t in zip(bars, tempos):
        ax.text(bar.get_width() + max(tempos) * 0.01, bar.get_y() + bar.get_height() / 2,
                f"{t:.2f}s", va="center", fontsize=9, fontweight="bold")
...
def grafico_agrupado():
    ...
    for i, nome in enumerate(nomes):
        ...
        ax.bar(x + offset, tempos, largura, label=nome, color=cor,
...
def grafico_escalabilidade():
    ...
    for nome, (cor, tempos) in RESULTADOS.items():
        ...
        ax.plot(VOLUMES_NUM, tempos, marker="o", label=nome, color=cor,
    ...
    ax.get_xaxis().set_major_formatter(
        mticker.FuncFormatter(lambda v, _: {1e6: "1M", 1e7: "10M", 1e8: "100M"}.get(v, v)))
...
def grafico_speedup():
    ...
    for bar, s in zip(bars, speed):
        ax.text(bar.get_width() + max(speed) * 0.01, bar.get_y() + bar.get_height() / 2,
                f"{s:.1f}x", va="center", fontsize=9, fontweight="bold")

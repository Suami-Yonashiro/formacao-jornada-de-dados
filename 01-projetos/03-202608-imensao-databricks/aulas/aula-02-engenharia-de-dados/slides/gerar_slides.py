...
def divisor(chapeu, titulo, linha):
    s = nova()
    barra = s.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(0), Inches(0), Inches(0.7), ALTURA
    )
    barra.fill.solid()
...
def linhas(chapeu, titulo, itens, rodape=None, mono=False):
    ...
    for i, item in enumerate(itens):
        ...
        texto(
            ...
            altura,
            tam_rot,
            ACENTO if destaque else BRANCO,
            True,
...
fluxo(
    ...
    [
        (
            "CLAUDE CODE",
            [
                "AI Dev Kit · 40 skills oficiais",
                "MCP · o workspace inteiro",
                "Asset Bundles (DABs)",
            ],
        ...
        (
            "GUARDRAILS",
            [
                "permissions deny no settings.json",
                "hook PreToolUse determinístico",
                "profile explícito, sempre",
            ],
        ...
        (
            "ESTEIRA",
            [
                "bundle validate a cada passo",
                "11 testes que interrompem o job",
                "6 deploys, um por prompt",
            ],
...
linhas(
    ...
    [
        (
            "Como poderíamos…",
            "Todo problema virou pergunta, sem ninguém propor solução ainda.",
        ),
        ("Meta de 2 anos", "Escrita no passado, com número. Meta sem número é desejo."),
        (
            "O que pode dar errado",
            "Risco levantado ANTES de construir. Depois fica caro.",
        ),
        ("Quem faz o quê", "O caminho da pergunta até a ação — e o caminho de volta."),
        (
            "A menor coisa que responde",
            "O protótipo. Não é o produto final: é o suficiente para saber se presta.",
            True,
        ),
    ],
...
linhas(
    "A PONTE",
    "O sprint não entrega\ncódigo. Entrega\nfeatures.",
    [
        (
            "O erro",
            "Jogar tudo num prompt só. Ele devolve alguma coisa — e você não sabe qual parte quebrou.",
        ),
        (
            "A saída certa",
            "De 5 a 6 features. Menos que 4 vira one-shot com outro nome; mais que 8 não cabe na noite.",
        ),
        (
            "Uma feature = um prompt",
            "Com começo, meio e fim. Você valida antes de seguir para a próxima.",
            True,
        ),
    ],
...
linhas(
    ...
    [
        (
            "databricks",
            "A CLI do Databricks. É por ela que tudo acontece daqui para frente.",
        ),
        (
            "gh",
            "A CLI do GitHub. Leva o repositório local para o remoto em um comando.",
        ),
        (
            "uv",
            "O gerenciador de pacotes Python. Rápido, e resolve o ambiente sozinho.",
        ),
        (
            "claude",
            "O Claude Code. É o quarto ambiente da semana — o único que CONSTRÓI.",
            True,
        ),
    ],
    mono=True,
    rodape="Conta no Databricks Free Edition: sem cartão, sem pegadinha. Só serverless.",
)
...
linhas(
    ...
    [
        (
            "Template",
            "Python. É o que traz src/, resources/, tests/ e o databricks.yml.",
        ),
        ("Jobs de exemplo", "Não. A gente vai escrever os nossos."),
        (
            "Serverless",
            "SIM — e não é preferência. O Free Edition só permite serverless.",
            True,
        ),
    ],
    mono=True,
    rodape="O Asset Bundle traz as boas práticas da engenharia de software para dentro da engenharia de dados.",
)
...
linhas(
    ...
    [
        ...
        (
            "uv venv --seed",
            "O --seed já instala o pip por padrão, e evita dor de cabeça depois.",
        ),
        ("uv sync", "Instala o que o template do bundle já trouxe.", True),
    ],
    mono=True,
    rodape="É o erro número 1 de quem tenta em casa: instalou tudo no 3.13 e nada funciona.",
)
...
linhas(
    ...
    [
        (
            "O que ele faz",
            "Abre o navegador, você autentica, e pronto — o profile fica salvo.",
        ),
        (
            "O efeito colateral bom",
            "Ele preenche sozinho o host lá no databricks.yml do bundle.",
        ),
        (
            "Sempre com --profile",
            "Nunca deixe o profile implícito. Um dia você deploya em prod sem querer.",
            True,
        ),
    ],
    mono=True,
    rodape="Dev e prod isolados, testes unitários, esteira de CI/CD — o bundle usa Terraform por baixo.",
)
...
linhas(
    ...
    [
        (
            "git init + commit",
            "O famoso first commit. Antes de a IA escrever a primeira linha.",
        ),
        (
            "gh repo create",
            "Leva o repositório local para o GitHub, sem abrir o navegador.",
        ),
        (
            "Por que agora",
            "Se a IA vai escrever código, você precisa de git diff e de git revert.",
            True,
        ),
    ],
    mono=True,
    rodape="O bundle já nasce com um README explicando como executar o projeto.",
)
...
linhas(
    ...
    [
        (
            "Onde achar",
            'Pesquise "AI Dev Kit Databricks". É o repositório oficial de solutions.',
        ),
        (
            "40 skills",
            "Por grupo: engenharia de dados, BI, machine learning, app developer.",
        ),
        ("Escopo", "Instale no PROJETO, não global. Cada projeto tem o contexto dele."),
        (
            "Skill × MCP",
            "Skill é injeção de CONHECIMENTO. MCP é AÇÃO — é o que conecta no workspace.",
            True,
        ),
    ],
...
linhas(
    "SETUP · PASSO 6 — E O MAIS IMPORTANTE",
    "Guard rails\nANTES do MCP",
    [
        (
            ".claude/settings.json",
            "Negue: bundle destroy, deploy --target prod, rm -rf, git push --force.",
        ),
        (".claude/hooks/", "Um script que bloqueia DROP, TRUNCATE e DELETE sem WHERE."),
        (
            "Por que hook",
            "Hook é DETERMINÍSTICO. Skill e MCP são probabilísticos: se ele bloqueia, bloqueia sempre.",
            True,
        ),
    ],
...
linhas(
    ...
    [
        (
            "1 · Raw",
            "Catálogo como código, Volume, e a conferência de chegada dos arquivos",
        ),
        (
            "2 · Bronze",
            "Os 10 CSVs viram Delta. Nenhuma limpeza — a sujeira é preservada",
        ),
        ("3 · Silver", "A limpeza, com o contrato de qualidade declarado na tabela"),
        (
            "4 · Gold",
            "Dimensões, fato, marts por diretoria e os 9 testes que quebram o job",
        ),
        ("5 · Dashboard", "AI/BI como código, versionado dentro do bundle"),
        (
            "6 · Agentes de IA",
            "Metadado, views de negócio e o Genie sobre dado limpo",
            True,
        ),
    ],
...
linhas(
    ...
    [
        (
            "Raw não é bronze",
            "Raw é ARQUIVO. Bronze é TABELA. O CSV fica byte por byte — é a prova.",
        ),
        (
            "Volume, não DBFS",
            "Volume é objeto do Unity Catalog: tem dono, permissão e linhagem.",
        ),
        (
            "Conferência de chegada",
            "A tarefa mais chata do pipeline, e a que mais salva emprego.",
            True,
        ),
    ],
...
linhas(
    ...
    [
        ...
        (
            "Tudo entra como texto",
            "Se inferir tipo, 15/10/2025 vira nulo e 309 CNPJs perdem o zero à esquerda.",
            True,
        ),
    ],
...
linhas(
    ...
    [
        (
            "A decisão da noite",
            "Quantidade negativa é devolução. Descartar infla o faturamento; sinalizar é o certo.",
        ),
        (
            "Deduplicar ≠ DISTINCT",
            "40 CNPJs com dois cadastros. O id é diferente, então DISTINCT não vê nada.",
        ),
        (
            "CONSTRAINT é contrato",
            "O Delta passa a RECUSAR a escrita que violar a regra. Ela vira da tabela, não do script.",
            True,
        ),
    ],
...
linhas(
    ...
    [
        (
            "O contrato antes do SQL",
            "Granularidade, dimensões, métricas e filtros. Escritos numa frase.",
        ),
        (
            "Um fato, vários marts",
            "O que separa um mart do outro é a dimensão dominante, nunca a tabela base.",
        ),
        (
            "Teste que não quebra",
            "…não é teste, é relatório. Se falhar, o job PARA.",
            True,
        ),
    ],
...
linhas(
    ...
    [
        (
            "Clicado",
            "Não tem diff, não tem revisão, não tem rollback. Se alguém apaga, acabou.",
        ),
        ("Em JSON no bundle", "Sobe junto com o deploy. Desfazer é git revert."),
        (
            "Métrica declarada uma vez",
            "Nenhuma tela mostra receita diferente da outra. É o fim da reunião travada.",
            True,
        ),
    ],
...
linhas(
    "PASSO 6 · AGENTES DE IA",
    "O mesmo dado,\noutra porta",
    [
        (
            "COMMENT é interface",
            "Não é documentação para humano. É o que o agente lê para DECIDIR a coluna.",
        ),
        (
            "View com nome de negócio",
            "Ninguém pergunta por fato_vendas. Perguntam por clientes em risco.",
        ),
        (
            "A regra que ele não adivinha",
            "Que dezembro é vale POR DESENHO do setor. Alguém precisa escrever.",
            True,
        ),
    ],
...
saida = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "aula-02-engenharia-de-dados.pptx"
)
prs.save(saida)

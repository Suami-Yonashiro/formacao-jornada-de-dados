...
def montar_features(referencia):
    ...
    fato = spark.table(f"{catalog}.gold.fato_vendas").filter(
        F.col("data_pedido") < F.lit(referencia)
    )

    oport = spark.table(f"{catalog}.silver.oportunidades").filter(
        F.col("data_abertura") < F.lit(referencia)
    )

    visitas = spark.table(f"{catalog}.silver.visitas").filter(
        F.col("data_visita") < F.lit(referencia)
    )
    ...
    rfm = (
        ...
        .withColumn(
            "margem_percentual",
            (F.col("margem_total") / F.nullif(F.col("valor_total"), F.lit(0))).cast(
                "double"
            ),
        )
    )
    ...
    gaps = (
        datas.withColumn("_anterior", F.lag("data_pedido").over(janela))
    ...
    recentes = (
        fato.filter(F.col("data_pedido") >= F.date_sub(ref, 90))
        .groupBy("cliente_id")
    ...
    crm_op = (
        ...
        .withColumn(
            ...
            (
                ...
                / F.nullif(F.col("_oportunidades"), F.lit(0))
            ).cast("double"),
        )
        .drop("_oportunidades")
    )
    ...
    crm_vis = (
        ...
        .agg(
            ...
            F.sum(F.col("gerou_pedido").cast("int"))
            .cast("double")
            .alias("_com_pedido"),
        )
        .withColumn(
            "conversao_visita",
            (F.col("_com_pedido") / F.nullif(F.col("visitas_90d"), F.lit(0))).cast(
                "double"
            ),
        )
        .drop("_com_pedido")
    ...
    por_marca = fato.groupBy("cliente_id", "marca").agg(
        F.sum("receita").alias("_receita")
    )
    concentracao = (
        por_marca.groupBy("cliente_id")
        .agg(
            F.max("_receita").cast("double").alias("_top"),
...
comprou = (
    ...
    .filter(
        (F.col("data_pedido") >= F.lit(CORTE_TREINO))

...
print(
    f"taxa base: {100 * taxa_base:.2f}%  —  de 200 ligações às cegas, "
...
for treino_idx, teste_idx in StratifiedKFold(
    5, shuffle=True, random_state=SEMENTE
).split(X, y):
...
print(
    f"score_propensao: {len(score)} clientes · "
...
holdout = pd.DataFrame(
    {"score": modelo.predict_proba(X_te)[:, 1], "comprou": y_te.values}
)
holdout["faixa"] = pd.qcut(
    holdout["score"].rank(method="first"),
    4,
    labels=["Fria", "Morna", "Quente", "Muito quente"],
).astype(str)

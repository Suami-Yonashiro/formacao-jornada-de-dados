...
def barra(s, cor=ACENTO):
    sh = s.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(0), Inches(0), Inches(0.22), ALTURA
    )
    sh.fill.solid()
...
def divisor(kicker, titulo, sub):
    ...
    n = titulo.count("\n") + 1
    texto(
        s, titulo, 1.15, 3.05, 11.3, 1.1 * n, 60, BRANCO, True, entrelinha=0.95, spc=-45
    )
...
def cartoes(s, y, h, itens, tam_rot=17, tam_corpo=12.5, mono_rot=False):
    ...
    for (x, yy, w, hh), item in zip(grade(y, h, len(itens)), itens):
        ...
        cor_b = (
            ALERTA if marca == "alerta" else (ACENTO if marca == "destaque" else BORDA)
        )
        tom = CARD_2 if marca else CARD
        cartao(
            s, x, yy, w, hh, tom=tom, borda=cor_b, largura_borda=1.5 if marca else 1.25
        )
        cor_r = (
        ...
        texto(
            ...
            cor_r,
            True,
            fonte=MONO if mono_rot else SERIF,
            entrelinha=1.05,
        )
        texto(
            s,
            item[1],
            x + 0.28,
            yy + 0.26 + nr * tam_rot / 72 * 1.22 + 0.16,
            w - 0.56,
            hh,
            tam_corpo,
            CINZA,
...
def faixas(s, y, itens, alt=0.62, folga=0.14, larg_esq=4.6, tam=13.5, cabecalho_=None):
    """Tabela de duas colunas em faixas. itens: (esq, dir) ou (esq, dir, marca)."""
    if cabecalho_:
        texto(
            s, cabecalho_[0], MARGEM + 0.34, y, larg_esq, 0.3, 11, ACENTO, True, spc=140
        )
        texto(
            s,
            cabecalho_[1],
            MARGEM + 0.34 + larg_esq,
            y,
            UTIL - larg_esq - 0.7,
            0.3,
            11,
...
def fluxo(s, y, etapas, h=0.95, quebrado=False, tam=13):
    ...
    for i, e in enumerate(etapas):
        ...
        texto(
            ...
            e,
            x + 0.16,
            y + (h - tam / 72 * 1.3 * (e.count("\n") + 1)) / 2,
            w - 0.32,
            h,
            tam,
            ALERTA if alvo else (ACENTO_CLR if destaque else BRANCO),
            True,
            fonte=MONO,
            alinha=PP_ALIGN.CENTER,
            entrelinha=1.3,
        ...
        if not ultimo:
            texto(
                s,
                "✗" if (quebrado and i == n - 2) else "→",
                x + w,
                y + h / 2 - 0.19,
                seta,
                0.4,
                17,
                ALERTA if (quebrado and i == n - 2) else ACENTO,
                True,
                alinha=PP_ALIGN.CENTER,
            )
...
def tabela(
    ...
    for i, linha in enumerate(linhas):
        ...
        for x, w, val in zip(xs, larguras, linha):
            ...
            cor_txt = (
                cor if marcada and x != xs[0] else (BRANCO if x == xs[0] else CINZA)
            )
            texto(
                s,
                str(val),
...
texto(
    s,
    "IMERSÃO AO VIVO · NOITE 3 DE 4 · 26/08 · 19H30",
    1.15,
    2.25,
    11.0,
    0.32,
...
texto(
    s,
    "Ciência de dados e agentes de IA",
    1.15,
    4.68,
    11.0,
    0.5,
...
texto(
    s,
    "Rota do Perfume · Databricks + MLflow + Claude Code",
    1.15,
    6.55,
    11.0,
    0.35,
...
texto(
    s,
    "“Luciano, a plataforma ficou boa. Dashboard bonito, o Genie responde tudo.",
    MARGEM + 0.55,
    y + 0.4,
    UTIL - 1.1,
    0.5,
...
texto(
    s,
    "Mas o meu time não vive de dashboard. Eu tenho 42 vendedores e, somando\ntodo mundo, dá umas 200 ligações boas por semana. No melhor mês.",
    MARGEM + 0.55,
    y + 1.08,
    UTIL - 1.1,
    1.0,
...
texto(
    s,
    "Eu tenho 3.000 clientes na base.",
    MARGEM + 0.55,
    y + 2.06,
    UTIL - 1.1,
    0.5,
...
nota(
    s,
    "Deixa a citação respirar. Leia em voz alta, sem pressa, e pare depois "
...
bloco(
    s,
    5.62,
    "Cada cliente é abordado, em média, uma vez a cada 15 semanas.\n"
...
s, y = cabecalho(
    "O limite do que a gente já tem", "Tudo que a gente construiu olha para trás"
)
faixas(
    s,
    y,
    ...
    alt=0.72,
    larg_esq=5.4,
    cabecalho_=("O QUE A GOLD RESPONDE", "O QUE O DIRETOR PRECISA"),
)
texto(
    s,
    "Dashboard mostra o passado com precisão. Ninguém liga para o passado.",
    MARGEM,
    6.42,
    UTIL,
    0.4,
...
s, y = cabecalho(
    "O que a maioria das empresas faz", "Três jeitos de escolher os 200 — todos ruins"
)
cartoes(
    s,
    y + 0.1,
    ...
    [
        (
            "Os maiores\nclientes",
            "Eles já compram. Você gasta ligação com quem ia comprar de qualquer jeito.",
        ),
        (
            "Os que sumiram\nhá mais tempo",
            "Muitos já foram embora. Você gasta ligação em cliente perdido.",
        ),
        (
            "A intuição\ndo vendedor",
            "Funciona para os 20 que ele lembra. E os outros 2.980?",
        ),
    ],
    tam_rot=19,
    tam_corpo=13.5,
...
bloco(
    s,
    5.72,
    "Todos os três são defensáveis numa reunião.\n"
    "E todos os três desperdiçam ligação.",
    h=1.0,
    tam=16,
)
...
bloco(
    s,
    y + 2.25,
    "Não é classificar quem compra e quem não compra. É ORDENAR.\n"
...
nota(
    s,
    "Fala: 'E sim, se a pergunta fosse sobre desconto, o modelo certo seria "
...
s, y = cabecalho(
    "A métrica que importa", "Não é acurácia.\nÉ quantos dos 200 compraram."
)
faixas(
    s,
    y,
    [
        ...
        (
            "Ligar para quem sumiu há mais tempo",
            "0     ·  esses já foram embora",
            "alerta",
        ),
...
bloco(
    s,
    5.78,
    "Isso tem nome: LIFT. Quantas vezes melhor que o aleatório.\n"
    "É o número que você leva para a reunião — não o AUC.",
    h=1.0,
    tam=16,
)
nota(
    s,
    "Todos medidos no workspace com seed 42, corte 2026-08-01, janela de 7 "
...
bloco(
    s,
    y + 3.3,
    "Cada prompt termina em DEPLOY, e o de baixo é a resposta do diretor.\n"
...
nota(
    s,
    "Deploy não é etapa de fim de projeto — é o que acontece toda vez que "
    "você termina alguma coisa. Por isso são três, e não um no final.",
)

# ── 9 · ML é camada, não projeto ──────────────────────────────────────
s, y = cabecalho(
    "Onde esse código vai morar",
    "ML é camada, não projeto à parte",
...
faixas(
    ...
    [
        (
            "12 → 15 tarefas",
            "O job de ontem ganha três. Continua sendo um bundle run só.",
        ),
        ("Os mesmos testes", "Modelo ruim quebra o pipeline, igual a dado nulo."),
        (
            "A mesma auditoria",
            "Faltou COMMENT numa coluna de feature? O job para.",
            "destaque",
        ),
    ],
    alt=0.64,
...
nota(
    s,
    "É assim que nasce o modelo que ninguém consegue colocar em produção: "
...
s, y = cabecalho("O que a noite 2 deixou pronto", "A matéria-prima já está na mesa")
codigo(
    s,
    y,
...
faixas(
    ...
    [
        (
            "4 dimensões + 1 fato",
            "cliente, produto, vendedor, calendário — e fato_vendas.",
        ),
        (
            "3 marts + 6 views",
            "vendedor, produto, financeiro. E as views que o Genie já lê.",
        ),
...
nota(
    s,
    "Abra isso no SQL Editor de verdade. São 30 segundos e é o que dá "
...
faixas(
    ...
    [
        ...
        (
            "1.111 + 223",
            "CNPJ pontuado e CNPJ com espaço em volta. Três formatos ao todo.",
        ),
        (
            "957 pedidos",
            "cancelados com valor_total = 0 — e o ITEM sem flag nenhuma.",
            "alerta",
        ),
...
bloco(
    s,
    6.02,
    "A bronze guardou tudo isso exatamente como veio.\n"
...
s, y = cabecalho(
    "O que acontece se ninguém limpar", "R$ 3,5 milhões de receita\nque nunca existiu"
)
codigo(
    s,
    y,
    "-- somando o item cru, como ele veio do ERP\n"
    "SELECT SUM(valor_bruto) FROM bronze.itens_pedido;   -- 105.890.448,42\n\n"
    "-- a gold, com os 957 pedidos cancelados fora\n"
    "SELECT SUM(receita)     FROM gold.fato_vendas;      -- 102.303.828,05",
    tam=12.5,
)
faixas(
    s,
    ...
    [
        ...
        (
            "Por que passa",
            "O pedido diz valor_total = 0. O item não diz nada.",
            "alerta",
        ),
        (
            "O que quebraria hoje",
            "'Quanto este cliente gasta' erraria em todo cliente que já cancelou.",
            "destaque",
        ),
    ],
    alt=0.62,
...
nota(
    s,
    "Este é o slide que amarra a noite 2 na noite 3: a feature valor_total do "
...
faixas(
    ...
    [
        (
            "NÃO é",
            "um programa com regras que alguém escreveu. Ninguém digita 'se atraso > 2 então quente'.",
        ),
        ("É", "uma função que recebe colunas e devolve um número entre 0 e 1."),
        (
            "Aprende",
            "olhando milhares de clientes do passado em que a resposta JÁ é conhecida.",
            "destaque",
        ),
        (
            "Devolve",
            "uma nota de propensão. Não uma certeza, não uma causa, não uma promessa.",
        ),
    ],
    alt=0.68,
    larg_esq=3.4,
...
bloco(
    s,
    y + 3.15,
    "Ele não descobre POR QUE o cliente compra.\n"
...
# ── 14 · o vocabulário ────────────────────────────────────────────────
s, y = cabecalho(
    "Quatro palavras que vão aparecer a noite inteira",
...
faixas(
    ...
    [
        (
            "Treino  ·  75%",
            "os clientes que o modelo VÊ. É olhando para eles que ele se ajusta.",
        ),
        (
            "Teste  ·  25%",
            "os que ele NÃO vê. A única nota honesta que existe.",
            "destaque",
        ),
    ],
    alt=0.66,
...
bloco(
    s,
    y + 3.9,
    "Nota tirada no treino é prova com o gabarito na mão.\n"
    "Todo número que a gente levar para a tela sai do teste.",
    h=0.95,
    tam=15,
)
...
# ── 15 · as premissas ─────────────────────────────────────────────────
s, y = cabecalho(
    "O que estamos assumindo — e onde isso quebra", "As cinco premissas do projeto"
)
faixas(
    s,
    y,
    [
        (
            "O passado se repete",
            "setembro se parece o bastante com agosto. Quebra em mudança de política comercial.",
        ),
        (
            "O futuro não entrou",
            "nenhuma coluna sabe o que houve depois do corte. É o vazamento, e é o próximo slide.",
        ),
        (
            "Uma linha por cliente",
            "clientes não se influenciam. Quebra se dois forem a mesma rede.",
        ),
        (
            "Treino parecido com o score",
            "quem eu pontuo se parece com quem eu treinei. Quebra com base nova.",
        ),
        (
            "Ordenar, não explicar",
            "'atraso alto anda junto com compra' não é 'atraso alto CAUSA compra'.",
            "destaque",
        ),
    ],
    alt=0.58,
...
texto(
    s,
    "Premissa não é detalhe teórico: é a lista do que conferir quando o "
    "modelo começar a errar.",
    MARGEM,
    6.35,
    UTIL,
    0.4,
...
nota(
    s,
    "Se alguém perguntar 'e se a empresa mudar a política de desconto?': "
...
s, y = cabecalho(
    "A palavra que descreve 80% do trabalho", "O que é feature engineering"
)
bloco(
    s,
    y,
    "Transformar o dado que existe em colunas que RESPONDEM à pergunta.\n"
...
faixas(
    ...
    [
        (
            "O que NÃO é",
            "escolher algoritmo, ajustar hiperparâmetro, procurar biblioteca nova.",
        ),
    ],
    alt=0.62,
    larg_esq=3.4,
...
nota(
    s,
    "É a parte que não dá para terceirizar: depende de saber como a empresa "
...
texto(
    s,
    "Uma linha por cliente. Vinte colunas descrevendo o comportamento dele.\n"
...
for (x, yy, w, h), (sig, perg, col) in zip(
    ...
    texto(
        s, col, x + 0.32, yy + 2.05, w - 0.64, 0.4, 13.5, ACENTO_CLR, True, fonte=MONO
    )
bloco(
    s,
    5.98,
    "Três colunas. Se você só tiver isso, já é melhor que a intuição.",
    h=0.72,
    tam=16,
)
...
bloco(
    s,
    5.68,
    "Mesma recência. Situações opostas.\n"
...
s, y = cabecalho(
    "O que a tabela de features tem dentro",
...
tabela(
    ...
    larguras=[3.9, 1.7, 2.0, 1.3, 1.4, 1.53],
    destaque=2,
    cor_destaque=ALERTA,
)
bloco(
    s,
    y + 2.5,
    "São 20 colunas assim. Nenhuma delas é opinião —\n"
    "todas saem de somar e dividir o que já aconteceu.",
    h=1.0,
    tam=16,
)
nota(
    s,
    "O terceiro cliente está há 545 dias sem comprar, com ciclo de 43. Dez "
...
texto(
    s,
    "Não vem de biblioteca nenhuma. Vem de entender como a empresa vende.\n"
...
s, y = cabecalho(
    "Como se sabe se o modelo é bom", "Esconda parte dos clientes.\nDepois confira."
)
mono(
    s,
    y,
    "2.815 clientes que já compraram\n"
    "  │\n"
    "  ├──  2.111  TREINO   o modelo vê. É olhando para eles que aprende\n"
    "  │\n"
    "  └──    704  TESTE    o modelo NÃO vê. Guardados no cofre",
    tam=13,
    h=2.3,
)
faixas(
    ...
    [
        ...
        (
            "O que se mede",
            "Dos 704 escondidos, o modelo acertou quem ia comprar?",
            "destaque",
        ),
    ],
    alt=0.66,
...
bloco(
    s,
    y + 4.15,
    "Todo número que a gente levar para a tela hoje sai dos 704.",
    h=0.7,
    tam=16,
)
...
texto(
    s,
    "Dado o comportamento de um cliente até hoje, qual a probabilidade\n"
...
s, y = cabecalho(
    "A parte que todo mundo acha que é o trabalho", "O algoritmo tem três linhas"
)
codigo(
    s,
    y,
    ...
    "score  = modelo.predict_proba(X_cliente)[:, 1]",
    tam=15,
)
faixas(
    s,
    ...
    [
        ...
        (
            ...
            "predict devolve 0 ou 1. A fila precisa de nota, não de sim ou não.",
            "destaque",
        ),
        (
            "random_state=42",
...
bloco(
    s,
    y + 3.9,
    "Estas três linhas são iguais em qualquer empresa do mundo.\n"
    "O que muda é o X — e o X foi o prompt 1.",
    h=1.0,
    tam=16,
)
nota(
    s,
    "É aqui que a sala espera a aula ficar difícil, e é o slide mais curto "
    "da noite. Diga isso em voz alta.",
)

# ── NOVO · quem comprou ontem vale zero ───────────────────────────────
s, y = cabecalho(
    "Dois clientes reais, dois extremos", "Quem comprou ontem\nnão compra hoje"
)
tabela(
    s,
    y,
...
texto(
    s,
    "Distribuição funciona por ciclo de reposição: o varejista acabou de "
...
nota(
    s,
    "Este é o slide para parar e deixar a sala olhar. Os dois números saíram "
...
texto(
    s,
    "Sorteie dois clientes: um que comprou e um que não.\n"
...
bloco(
    s,
    y + 4.25,
    "AUC compara FILAS. Lift conta LIGAÇÕES.\n"
...
nota(
    s,
    "Os quatro números são as respostas da sala, medidas. O 0,37 é o momento "
...
s, y = cabecalho(
    "O erro que mais mata modelo na vida real",
    "Prever a semana que vem\ncom informação da semana que vem",
)
mono(
    s,
    y,
    "|---- features até 31/07 ----|CORTE|---- comprou até 07/08? ----|\n"
    "                              01/08/2026",
    tam=13,
    h=1.28,
)
cartoes(
    ...
    [
        (
            "Errado",
            "Features com todo o histórico\nAUC 0,98 — e quebra em produção",
            "alerta",
        ),
...
bloco(
    s,
    y + 3.4,
    "AUC alto demais é sinal de vazamento, não de competência.",
    h=0.72,
    tom=CARD_2,
    borda=ALERTA,
    tam=16,
...
s, y = cabecalho(
    "Por que o modelo entra nos mesmos testes de ontem",
...
faixas(
    ...
    [
        ...
        (
            "Modelo ruim",
            "Devolve nota para todo mundo, na faixa certa, sem erro nenhum.",
        ),
        (
            "O resultado",
            "Pipeline verde, dashboard atualizado, e a lista errada por seis meses.",
            "alerta",
        ),
    ],
    alt=0.68,
...
codigo(
    ...
    'assert lift_top200 >= 2.5,           "a fila não se paga"',
    tam=12.5,
)
texto(
    s,
    "O teste que quase ninguém escreve é o primeiro: o modelo ganha do que "
    "a gente já fazia de graça?",
    MARGEM,
    6.52,
    UTIL,
    0.4,
...
nota(
    s,
    "Quebre um destes de propósito ao vivo. Teste que ninguém viu falhar não "
    "convence ninguém.",
)
...
divisor(
    "Ainda no prompt 2 · terceiro passo",
    "MLflow",
...
bloco(
    s,
    y,
    "Uma biblioteca aberta — criada pela Databricks — que registra tudo o que\n"
...
faixas(
    ...
    [
        (
            "No Databricks",
            "já vem instalado, e o Registry É o Unity Catalog — o mesmo do dado.",
            "destaque",
        ),
    ],
    alt=0.62,
...
s, y = cabecalho(
    "Quando você não estiver mais no projeto", "“Esse modelo ainda está bom?”"
)
faixas(
    s,
    y,
    ...
    alt=0.66,
    larg_esq=6.2,
    cabecalho_=("PERGUNTA", "SEM MLFLOW"),
)
bloco(
    s,
    6.0,
    "O MLflow não faz a empresa vender mais. O MODELO faz.\n"
...
codigo(
    ...
    '        registered_model_name="propensao_compra")',
    tam=12,
)
texto(
    s,
    "Repare no lift_top200. Essa é a métrica que responde a pergunta do "
...
s, y = cabecalho(
    "Por que isso importa NESTE projeto", "Três motivos concretos,\nnão boas práticas"
)
faixas(
    s,
    y,
    [
        ...
        (
            ...
            "score_propensao grava o número da versão. Dá para auditar a lista de três semanas atrás.",
            "destaque",
        ),
        (
...
codigo(
    s,
    y + 2.9,
    "# quem consome nunca precisa saber o número da versão\n"
    'mlflow.sklearn.load_model("models:/...propensao_compra@prod")',
    tam=13,
)
texto(
    s,
    'Sem isso, "o modelo está pior esse mês" é uma conversa sem prova de '
...
s, y = cabecalho("Onde os projetos de ML morrem", "O último metro é o mais difícil")
fluxo(
    s,
    y + 0.15,
    ["Dado", "Modelo", "Score na\ntabela", "Alguém\nligar"],
    h=1.05,
    quebrado=True,
)
...
texto(
    s,
    "“Já vi empresa com modelo excelente rodando há dois anos, AUC de 0,89,\n"
...
texto(
    s,
    "O último metro é o mais difícil. E é o que o agente resolve.",
    MARGEM,
    y + 3.55,
    UTIL,
    0.4,
...
tabela(
    ...
    [
        ...
        [
            "Perfumaria Aurora",
            "0,88",
            "4 pedidos nos últimos 90 dias. Ciclo curto — não esfrie.",
        ],
        [
            "Bella Vita",
            "0,87",
            "Mais da metade das visitas viram pedido (67%). Vale a ida.",
        ],
    ],
    larguras=[3.3, 1.2, 7.33],
    tam=12.5,
    alt=0.66,
)
bloco(
    s,
    y + 2.7,
    "O vendedor não recebe um número. Recebe um nome, uma ordem\n"
    "e uma frase que ele entende antes de discar.",
    h=1.0,
    tam=16,
)

# ── NOVO · onde a lista aparece ───────────────────────────────────────
s, y = cabecalho("A pergunta que todo mundo faz", "E onde eu vejo esses 200?")
cartoes(
    s,
    y,
    2.5,
...
bloco(
    s,
    y + 2.75,
    "As três leem a MESMA tabela. Nenhuma delas tem número inventado:\n"
...
nota(
    s,
    "Abra o dashboard primeiro — é o que responde a pergunta do diretor. "
...
faixas(
    ...
    alt=0.68,
    larg_esq=5.0,
    cabecalho_=("FERRAMENTA", "O QUE FAZ"),
)
bloco(
    s,
    6.02,
    "Regra do sistema: use sempre as ferramentas, nunca invente número.\n"
...
s, y = cabecalho(
    "O que o vendedor abre na segunda de manhã", "A resposta para o diretor"
)
...
texto(
    s,
    "Os 200 de maior score da base inteira — e a fatia de cada vendedor "
    "varia de 2 a 10.",
    MARGEM,
    6.42,
    UTIL,
...
nota(
    s,
    "Se perguntarem por que não é a mesma cota para todo mundo: porque a "
...
s, y = cabecalho("A lista não repete a mesma desculpa", "Seis motivos, 200 contatos")
tabela(
    s,
    y,
...
texto(
    s,
    "Se os 200 saíssem com o mesmo motivo, a lista viraria enfeite. "
...
bloco(
    s,
    6.28,
    "Mesmo time. Mesmas 200 ligações. Mais de quatro vezes o resultado.",
    h=0.68,
    tam=16,
)
...
saida = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "aula-03-ciencia-de-dados.pptx"
)
prs.save(saida)

...
rotulo = (
    fato.filter((F.col("data_pedido") >= CORTE) & (F.col("data_pedido") <= FIM_JANELA))
    .select("cliente_id")
    .distinct()
    .withColumn("comprou_7d", F.lit(1))
)
...
honesto = (
    ...
    .groupBy("cliente_id")
    .agg(
        F.datediff(F.lit(CORTE), F.max("data_pedido")).alias("recencia_dias"),
...
vazado = (
    fato.groupBy(  # ← sem o filtro. É a única diferença para a célula anterior.
        "cliente_id"
    )
    .agg(
        F.datediff(F.lit(CORTE), F.max("data_pedido")).alias("recencia_dias"),
...
Xv_tr, Xv_te, yv_tr, yv_te = train_test_split(
    Xv, yv, test_size=0.25, stratify=yv, random_state=42
)
...
comparacao = pd.DataFrame(
    {
        ...
        "clientes_com_recencia_negativa": [
            (honesto.recencia_dias < 0).sum(),

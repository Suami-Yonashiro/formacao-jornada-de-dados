...
CATEGORIAS = {
    ...
    "Eau de Parfum": (
        ("Nadir", "Sahra", "Layali", "Mizan", "Qamar", "Rihan", "Dahab"),
        89.00,
        420.00,
        0.44,
    ),
    "Óleo Concentrado": (
        ("Nadir", "Sahra", "Attar Real", "Mizan", "Zahir"),
        45.00,
        260.00,
        0.52,
    ),
...
MOTIVOS_PERDA = [
    ...
    "Já compra de outro importador",
    "Prazo de entrega",
    "Sem contato",
    "Fechou com concorrente",
    "Mix de fragrâncias não atende",
    "Pediu exclusividade de marca",
]
...
def fator_sazonal(d):
    ...
    base = {
        ...
        2: 0.74,
        3: 0.88,
        4: 1.32,  # abril: reposição p/ Dia das Mães
        5: 0.96,
        6: 1.24,  # junho: Dia dos Namorados
        7: 0.82,
        8: 0.86,
        9: 0.94,
        10: 1.42,
        ...
        m
    ]  # outubro: reposição p/ Black Friday
...
def escrever(caminho, cabecalho, linhas):
    with open(caminho, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerows(linhas)
    print(
        f"  {os.path.basename(caminho):24} {len(linhas):>8,} linhas".replace(",", ".")
    )
...
def main():
    ...
    for i in range(1, N_VENDEDORES + 1):
        ...
        if rng.random() < 0.14:
            desl = (
                INICIO + timedelta(days=rng.randint(90, (FIM - INICIO).days - 30))
            ).isoformat()
    ...
    for c in duplicados:
        ...
        linhas.append(
            [
                prox_id,
                formatar_cnpj(c["cnpj"], fmt2),
                c["razao"].upper(),
                c["segmento"],
                c["cidade"],
                c["uf"],
                c["bairro"],
                c["cadastro"].isoformat(),
                "S",
            ]
        )
        prox_id += 1
    ...
    for c in clientes:
        ...
        if rng.random() < 0.22:
            troca = ini + timedelta(
                days=rng.randint(60, max(90, (FIM - ini).days - 30))
            )
            carteira.append(
                [
                    cid_carteira,
                    ...
                    v["id"],
                    ini.isoformat(),
                    (troca - timedelta(days=1)).isoformat(),
                ]
            )
            cid_carteira += 1
            v2 = rng.choice(vendedores)
            carteira.append([cid_carteira, c["id"], v2["id"], troca.isoformat(), ""])
            cid_carteira += 1
            carteira_atual[c["id"]] = [
                (ini, troca - timedelta(days=1), v["id"]),
                (troca, FIM, v2["id"]),
            ]
        else:
    ...
    for cat, (marcas, pmin, pmax, margem) in CATEGORIAS.items():
        ...
        for _ in range(qtd):
            ...
            if rng.random() < 0.18:
                lanc = (
                    INICIO + timedelta(days=rng.randint(60, (FIM - INICIO).days - 90))
                ).isoformat()
            produtos.append(
                {
    ...
    for c in clientes:
        if rng.random() < 0.11:
            churn[c["id"]] = INICIO + timedelta(
                days=rng.randint(120, (FIM - INICIO).days - 40)
            )
    ...
    for c in clientes:
        ...
        while d <= FIM:
            ...
            canal = rng.choices(
                ["Visita", "Telefone", "App", "WhatsApp"],
                weights=[0.44, 0.19, 0.23, 0.14],
            )[0]
            # 3,5% cancelados
            ...
            status = (
                ...
                if cancelado
                    ...
                    ["Faturado", "Entregue", "Em separação"], weights=[0.34, 0.62, 0.04]
                )[0]
            )
            ...
            if rng.random() < 0.06 and prod_inativos:
                escolha = escolha + rng.sample(
                    prod_inativos, min(2, len(prod_inativos))
                )
            skus, vistos = [], set()
            tentativas = 0
            ...
            for p in skus:
                qtd = rng.choices(
                    [1, 2, 3, 4, 6, 8, 12],
                    weights=[0.28, 0.24, 0.17, 0.12, 0.10, 0.06, 0.03],
                )[0]
                if c["porte"] == "G":
                    qtd *= rng.randint(1, 2)
                # sazonalidade também no volume, não só na frequência
                qtd = max(1, int(round(qtd * fator_sazonal_mes(d))))
                desc_pct = rng.choices(
                    [0, 0.03, 0.05, 0.08, 0.12, 0.18],
                    weights=[0.42, 0.18, 0.15, 0.12, 0.09, 0.04],
                )[0]
            ...
            # ---- pagamento ----
            if not cancelado:
                forma_idx = rng.choices(
                    range(len(FORMAS_PAGAMENTO)), weights=pesos_pag
                )[0]
                nome_f, _, prazo, taxa = FORMAS_PAGAMENTO[forma_idx]
                parcelas = 1
                if nome_f == "Cartão de crédito":
                    parcelas = rng.choices(
                        [1, 2, 3, 4, 6], weights=[0.5, 0.2, 0.15, 0.1, 0.05]
                    )[0]
                ...
                if venc > FIM:
                    ...
                    if r < 0.86:
                        st_pag = "Pago"
                        atraso = rng.choices(
                            [0, 1, 3, 7, 15], weights=[0.62, 0.16, 0.12, 0.07, 0.03]
                        )[0]
    ...
    while d <= FIM:
        for p in rng.sample(produtos, min(80, len(produtos))):
            ...
            if rng.random() < 0.11:
                saldo = 0
            estoque.append([d.isoformat(), p["sku"], saldo, "S" if saldo == 0 else "N"])
        d += timedelta(days=7)
    escrever(
    ...
    for c in clientes:
        n_op = rng.choices(
            [0, 1, 2, 3, 4, 6], weights=[0.18, 0.27, 0.23, 0.16, 0.10, 0.06]
        )[0]
        for _ in range(n_op):
            ...
            etapa = rng.choices(
                ETAPAS_FUNIL, weights=[0.14, 0.15, 0.17, 0.13, 0.27, 0.14]
            )[0]
            ...
            oportunidades.append(
                [
                    ...
                    c["id"],
                    vend,
                    rng.choice(ORIGENS_OPORTUNIDADE),
                    abertura.isoformat(),
                    etapa,
                    prob,
                    valor,
                    d_fech.isoformat() if d_fech else "",
                    (d_fech - abertura).days if d_fech else "",
                    motivo,
                ]
            )
            op_id += 1
    ...
    for c in clientes:
        ...
        while d <= FIM:
            ...
            if d.weekday() >= 5:
                continue
            vend = vendedor_do_cliente(c["id"], d)
            res = rng.choices(RESULTADO_VISITA, weights=[0.46, 0.24, 0.13, 0.09, 0.08])[
                0
            ]
